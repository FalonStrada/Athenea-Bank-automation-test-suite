...
test('TC-01 Verificación de elementos visuales', async ({ page }) => {
...
test('TC-7 Verificar Registro exitoso con datos válidos verificando respueste de la API', async ({ page }) => {
  await test.step('Completar el formulario de registro con datos válidos', async () => {
    const email = (TestData.usuarioValido.email.split('@')[0]) + Date.now().toString() + '@' + TestData.usuarioValido.email.split('@')[1];
    TestData.usuarioValido.email = email;
    await registerPage.completarFormularioRegistro(TestData.usuarioValido);
  ...
  const responsePromise = await page.waitForResponse('http://localhost:3000/api/register');
...
test('TC-8 Generar signup desde la API', async ({ page, request }) => {
  const email = (TestData.usuarioValido.email.split('@')[0]) + Date.now().toString() + '@' + TestData.usuarioValido.email.split('@')[1];
...
test('TC-9 Verificar comportamiento del front ante un error 500 en el registro', async ({ page }) => {
  const email = (TestData.usuarioValido.email.split('@')[0]) + Date.now().toString() + '@' + TestData.usuarioValido.email.split('@')[1];
